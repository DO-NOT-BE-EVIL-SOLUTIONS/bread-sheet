...
| `POST` | `/products/upload-image` | Registered | Multipart image → plausibility/abuse gate → S3 upload. `422 { error: 'image_rejected', reason }` if rejected (nothing stored). For `kind=product` returns `{ imageKey, name, brand, genericName }`; for `kind=label` returns `{ imageKey }`. `imageKey` is the `processed/{uuid}.jpg` S3 object key — echoed back as `productImageKey` in the submission. `requireRegistered` since ADR 0005 L-1 — the plausibility gate runs a paid Gemini call before anything is persisted, so anonymous sessions are turned away before they can spend it |
...
**Thinking disabled (ADR 0005 § implementation step 2).** Both call sites set `config.thinkingConfig = { thinkingBudget: 0 }`. Gemini 3.x bills internal reasoning as output tokens at the full rate, and neither the plausibility gate (a four-field classification) nor label extraction (structured field reads) benefit from it — measured at ~\$0.0036/call with thinking off vs. ~\$0.0126/call at the model's default thinking budget, the ~3.5× difference the `GEMINI_DAILY_CALL_CAP` (ADR 0005 § L2) sizing is set against. Every Gemini response also runs through `services/geminiUsage.ts` (`logGeminiUsage`), which logs one `info` line (`gemini:usage`: `operation`, `inputTokens`, `outputTokens` — `candidatesTokenCount` + `thoughtsTokenCount` folded together — and `usdCost` at the \$1.50/\$9.00 per-M rates above) per call, independent of GCP billing console latency; a missing `usageMetadata` logs a `warn` rather than throwing.
