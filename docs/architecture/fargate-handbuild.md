...
> **Superseded in one respect (2026-09-08).** Everything below describes the stack as it was built,
> including the **ALB** ingress. That ingress no longer exists: `dev` now runs API Gateway HTTP API →
> VPC Link v2 → Cloud Map → Fargate, with no load balancer at all
> ([ADR 0003](../architecture-decision-records/0003-always-on-production-cost-architecture.md), and
> [infrastructure.md](infrastructure.md) for the current shape). This file is kept as the build
> record — it is deliberately *not* rewritten, since its value is the history of how the stack was
> learned. Read the ALB sections as "how it used to work", not as instructions.
...
|---|---|---|---|
| A1 | **Migrate DB auth to keyless IAM** — ✅ **Code done.** `databaseConfig.ts` now supports `DB_AUTH=iam`: creates an `@aws-sdk/rds-signer` `Signer` and returns an async `password` callback for the pg.Pool. `scripts/start.sh` mints a token for the Prisma migration engine (which can't use the callback) and injects it into `DATABASE_URL` before `db:deploy`, via `node scripts/rds-token.mjs --database-url` — the script assembles the URL so the token gets percent-encoded (raw, its own `/` and `?` break the userinfo and Prisma answers `P1013: invalid port number`). `scripts/rds-token.mjs` is the standalone token minter; with no flag it prints the raw token for a `psql` prompt. Terraform: task role gets `rds-db:connect` scoped to the DB resource ID + IAM user; task-def env moves to `DB_AUTH=iam` + `DATABASE_URL` without a password (no more SSM `DATABASE_URL` secret in the task-def `secrets`). **Manual step done (2026-09-08):** the `breadsheet_iam` Postgres user was created on RDS (`CREATE USER breadsheet_iam WITH LOGIN; GRANT rds_iam TO breadsheet_iam; GRANT ALL PRIVILEGES ON DATABASE breadsheet TO breadsheet_iam; GRANT ALL ON SCHEMA public TO breadsheet_iam;`) — see [infrastructure.md § RDS IAM bootstrap](infrastructure.md#rds-iam-bootstrap-required-on-any-fresh-instance) for why the schema grant is separate on PG 15+. | App deployed and serving over the SSM password first. ✅ | [ADR 0002](../architecture-decision-records/0002-rds-database-credentials.md) |
