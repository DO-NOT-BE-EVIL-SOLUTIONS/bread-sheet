...
- `products/` — Add Product flow business logic (API helpers, on-device OCR wrapper, image picker + processing, shared constants/types) plus the P5-006 edit-form logic (`edit-form.ts`: pre-population, changed-fields diff, correction payload) and the P6-006 barcode validation (`barcode.ts`). Screens import from here; see `docs/architecture/frontend.md#product-submission-ticket-p5-002`.
...
**Manual barcode entry (P6-006):** `components/manual-barcode-sheet.tsx` is a modal numeric-entry sheet that navigates to `/(app)/product/<code>` — the same destination a scan produces, so every downstream state (found, pending, the 404 add-product screen, the anonymous sign-up gate) is reached identically and **no backend surface was added**. It is mounted from three places: the scan tab (below the viewfinder, rendered in *every* camera-permission state — the camera-free path must not require the camera), the Home tab header "＋", and the product screen's `400` branch (pre-filled, submitting via `router.replace`). Validation lives in `features/products/barcode.ts` and mirrors the server's `^\d{8,13}$` exactly, with typed reasons (`empty` / `non-digit` / `too-short` / `too-long`) so length and non-digit mistakes read as different errors. Separators are stripped as the user types; letters are kept so they can be *reported* rather than silently deleted. The scanner also recognises `code128` and `itf14` now; a code that reads but fails the pattern opens the sheet pre-filled with its digits.
...
1. `cors` — mounted **first, ahead of every gate below it**, and that order is load-bearing. `cors` only adds response headers, so it grants no access on its own; but a rejection that carries no `Access-Control-Allow-Origin` is unreadable to a browser — `fetch` rejects with a bare `TypeError`, which `lib/api.ts` maps to `NetworkError` and the app renders as "you appear to be offline". With `requireOriginSecret` mounted above `cors`, the Phase 2 header mismatch therefore showed up in the web client as *offline* on every screen while native correctly reported `403`. Origins come from `ALLOWED_ORIGINS` (comma-separated).
2. `requireOriginSecret` on `/api/*` (ADR 0005 Phase 2) — 403s any request lacking the `X-Origin-Verify` header **the CloudFront distribution sends as a `custom_header` on every request it forwards** (`terraform/dev-geo-restriction.tf`). It is deliberately *not* inserted by the WAF: AWS WAF prefixes every header it inserts with `x-amzn-waf-`, so a WAF-inserted `x-origin-verify` arrives as `x-amzn-waf-x-origin-verify` and never matches. A no-op when `ORIGIN_VERIFY_SECRET` is unset (local dev, tests, any stage with no CloudFront in front) — deliberately not fail-fast, unlike this file's other config vars.
3. `requestLogger` — emits one structured `request:start` (debug) and one `request:finish` (info/warn/error depending on status) line per request with `method`, `path`, `status`, `durationMs`, `userId`, `isAnonymous`, `ip`, and `x-request-id` (when supplied). Mounted before rate-limiting so even throttled requests are recorded.
4. Rate limiting: `apiLimiter` (100 req/15min) on `/api/*`, `authLimiter` (10 req/hr) on auth endpoints
5. `requireAuth` — verifies Supabase Bearer token, injects `user` into `req` (including `isAnonymous` flag derived from the JWT `is_anonymous` claim)
6. `requireRegistered` — composable second-layer guard for contribution routes; rejects anonymous sessions with `403 { error: 'Registration required' }`. Applied after `requireAuth` on `POST /api/products`, `POST /api/products/upload-image` (ADR 0005 L-1 — the plausibility gate runs a paid Gemini call before anything is persisted, so anonymous sessions never reach it), `POST /api/products/extract-label`, both `POST`/`DELETE /api/products/:barcode/verify`, and all P5-006 edit write paths (`PATCH /api/products/:barcode`, `POST /api/products/:barcode/edits`, `POST`/`DELETE /api/products/edits/:editId/votes`, `POST /api/products/edits/:editId/dismissals`).
7. `requestDeadline()` — 25 s handler deadline, mounted on the two synchronous Gemini routes (`POST /api/products/upload-image`, `POST /api/products/extract-label`). Answers `503 { code: 'request_timeout' }` if nothing has responded by then. It cannot *cancel* the handler — Express has no mechanism for that — so the handler may finish later and try to write to an already-sent response; `errorHandler` returns early on `res.headersSent` for exactly that reason. Paired with `withGeminiDeadline()` (`services/geminiDeadline.ts`), which bounds the model call itself at 20 s via `AbortSignal` and raises `503 { code: 'upstream_timeout' }`. Every *other* failure of that call is wrapped as `502 { code: 'upstream_error' }` — necessary because `@google/genai` throws an `ApiError` carrying Google's own HTTP status and `errorHandler` trusts `err.status`, so an unwrapped Vertex `404` ("model not published in this region") reached clients as our `404`, which in this API means "product not found". A dependency failing is never a 4xx. The budgets are nested — 20 s call < 25 s handler < the 30 s API Gateway integration timeout that ADR 0003's ingress would impose — so the innermost always fires first and a slow Gemini response is a clean, retryable 503 rather than an opaque 504 with the upload lost. Both 5xx bodies collapse to generic copy, so `code` is the only part a client can branch on.
8. Controllers handle request/response
...
The dev cloud environment runs on **ECS Fargate** (Terraform-provisioned: VPC + API Gateway HTTP API + ECS + RDS + S3 + image-resizer Lambda; **no load balancer** since ADR 0003 — TLS terminates at API Gateway outside the VPC and traffic reaches the task through a VPC Link v2 + Cloud Map SRV registration), deployed by push-based CD from GitHub Actions — no ArgoCD, no pull-reconcile loop. Database migrations run as a container-command step (`scripts/start.sh` → `npm run db:deploy`) before the server process starts. The server **container image** is published to **GitHub Container Registry** (`ghcr.io/fabelhaft-io/bread-sheet-server`, free public package) by `.github/workflows/build-image.yml` — not ECR. The earlier EKS design was never applied to this account and its Terraform (`eks.tf`, `irsa.tf`, `terraform/k8s/`) has been deleted from the repo (recoverable from git history) — see `docs/architecture/fargate-handbuild.md` for the migration record.

**Terraform layout (`terraform/`):** one root, one environment today (`environments/dev.tfvars` + `dev.s3.tfbackend`) — VPC/ECS/RDS in `network.tf`/`ecs.tf`/`rds.tf`, ingress in `api-gateway.tf` (HTTP API + VPC link + integration + `$default` route/stage + custom domain + access logs) and `service-discovery.tf` (Cloud Map private DNS namespace + **SRV** service), DNS + ACM in `dns.tf`, IAM (execution/task/deployer roles + GitHub OIDC) in `iam.tf`, GCP WIF in `gcp-wif.tf`, the images bucket's CloudFront distribution + OAC + WAF ACL (ADR 0005 L5) in `cloudfront.tf`, the RDS-stop budget action + GCP billing-detach Cloud Function (ADR 0005 L4) in `backstops-budget.tf` + `functions/billing-killswitch/`. The task and VPC-link security groups use **standalone `aws_vpc_security_group_*_rule` resources, never inline blocks** — inline rules cycle (each group references the other) and deadlock when a referenced group changes identity; see `infrastructure.md` § Security groups. State is an S3 remote backend with per-env keys (`backend.tf` + `environments/<env>.s3.tfbackend`). The ECS task role accesses S3 via IAM, and Google Cloud (Vision/Vertex) via **Workload Identity Federation** — both keyless (no static keys). Local dev does **not** use Terraform at all — `scripts/localstack-init.sh` provisions the S3 bucket + Lambda directly against LocalStack on `docker compose up`. See `docs/architecture/infrastructure.md` for the bootstrap + apply runbook.
...
# Daily cap on Gemini calls (ADR 0005 L2 — cost blast radius). Required, positive integer,
# no default, whenever VISION_MODE=llm or PLAUSIBILITY_MODE=gemini; unused (and unread) in
# mock modes. Enforced by services/geminiQuota.ts via a Postgres row (GeminiDailyUsage),
# reserved BEFORE the Gemini call — past the cap the call never happens and the endpoint
# answers 503 { code: 'daily_quota_exhausted' }. dev: 100 at launch, raised to 300 once
# thinking-disabled cost was confirmed (ADR 0005 § implementation steps 2 and 8, both done
# 2026-09-09) — 300/day × ~$0.0036/call blended ≈ $32/mo.
GEMINI_DAILY_CALL_CAP=300
...
- `0003-always-on-production-cost-architecture.md` — *Accepted, being implemented.* Always-on ingress: the ALB is dropped for API Gateway HTTP API → VPC Link v2 → Cloud Map (keeps Fargate warm; Lambda rejected on cold starts, App Runner on GHCR support). **Both stages run the same ingress** — `dev` is rebuilt onto it first as the rehearsal, then `prod` is stamped from the same Terraform root via a second `.tfvars`, so there are no conditional ingress resources. Step 0's blocking gate is **cleared**: the nested timeout budget (20 s Gemini call / 25 s handler, `services/geminiDeadline.ts` + `middlewares/requestDeadline.ts`) makes API Gateway's non-increasable 30 s ceiling structurally unreachable, so a slow model call is a retryable `503` rather than an opaque `504` with the upload lost. `dev` staying permanent rather than ephemeral is a deliberate departure from the ADR's option S-B
...
- `0005-cost-blast-radius-and-emergency-stop.md` — *Accepted, Phase 1 and Phase 2 implemented (2026-09-09, same day; Phase 2's app-code piece is on a feature branch pending a `dev` image rebuild).* What actually stops the bill when the API is flooded. HTTP APIs support no API keys, usage plans, per-client throttling, WAF or resource policies (ADR 0003's price for dropping the ALB), and API Gateway bills throttled 429s — so the only native gateway control is stage/route throttling. **The two fears are independent:** ADR 0003 added exactly one per-request AWS cost, bounded by ten lines of HCL; the frightening tail is unbounded Gemini spend from a public upload endpoint, which predates ADR 0003 and which the ALB never protected. **L1 bounds the AWS bill; only L2 bounds the Google one** — Vertex quotas are per-minute, and a 1 rps upload route still permits 86,400 `gemini-3.5-flash` calls/day (~\$311–1,089 at \$1.50/\$9.00 per M; no `thinkingConfig` is set anywhere in `server/src`, and setting it moves the affordable cap by 3.5×). **Sizing rule:** caps and `budget_limit_usd = 45` are chosen together against two bounds — a 2-day *detected* worst case under budget, and a 30-day *undetected* worst case under 2× budget — which is why a **detection layer D** (AWS Cost Anomaly Detection + CloudWatch alarms on API Gateway `Count` and a log-metric-filter `GeminiCalls` + a GCP budget alert, all into `aws_sns_topic.billing_alerts`) is in scope: it is what lets the caps be loose enough to develop against. **Phase 1 scope:** L-1 (`requireRegistered` on `POST /api/products/upload-image` — one line; guests are already turned away client-side and an anonymous upload can never become a product), L1 (stage throttle **5 rps / burst 25** — burst sized for the Maestro CI job, which runs against `dev` on every PR — plus an explicit upload route at 1 rps that exists only to be throttled), L2 (Postgres-backed daily Gemini counter, `GEMINI_DAILY_CALL_CAP` env var with no default, **300/day** on `dev` — started at the 100/day interim value, raised once thinking-disabled cost was measured; reserve-before-call, failures count, fail closed; `503 daily_quota_exhausted`), D, L5 and L4 (GCP billing detach at \$40) — about a day and a half, taking the sustained-attack worst case from ≥\$47,733/mo to **\$37.34/mo detected, \$84.18/mo undetected**, baseline included. **L5** solves what was an accepted residual: S3 image egress (`s3.tf:29`, `Principal: "*"`, unbounded at \$0.09/GB) goes behind a CloudFront distribution with OAC on a **flat-rate Free plan** — \$0/mo, 1M req + 100 GB, **no overage charges ever** (over the allowance AWS degrades delivery, it does not bill); a WAF web ACL is mandatory for the plan; the hosted zone is deliberately *not* attached (DNS overage can move an attached zone back to pay-as-you-go). Plain pay-as-you-go CloudFront is *not* a control (1.8% cheaper under attack — its requests cost 3× an S3 GET). **Phase 2, decided and sequenced after Phase 1:** a second Free-plan distribution in front of the API (quota is 3 Free plans per account, 1 apex domain per plan — the "two plans" question is closed) for **per-IP rate limiting** (the control the HTTP API lost, restored at the edge for \$0 and exempt from the allowance) and WAF geo-restriction at country level `DE` (never `DE-BW`: German mobile carriers egress centrally and subdivision lookups would lock out the real team), with `disable_execute_api_endpoint`, a header-insertion shared secret, a `us-east-1` cert and a CI allow path. On a flat-rate plan geo-restriction *is* a cost control because blocked requests never become API Gateway calls; on pay-as-you-go it was not (\$1.20/M vs \$1.00/M). The panic Lambda (L3) stays documented, not built
