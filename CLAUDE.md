...
- `0005-cost-blast-radius-and-emergency-stop.md` — *Proposed.* What actually stops the bill when the API is flooded. HTTP APIs support no API keys, usage plans, per-client throttling, WAF or resource policies (ADR 0003's price for dropping the ALB), and API Gateway bills throttled 429s — so the only native control is stage/route throttling. Adopts a five-layer stop (Vertex quota → stage throttle → in-app daily Gemini cap → CloudWatch-alarm panic Lambda → GCP billing detach + Budget Action), because no single layer covers all three vendors: the largest per-unit cost is Gemini, billed by *Google*, invisible to `budget.tf`. Scope is L-1 (`requireRegistered` on `POST /api/products/upload-image` — one line; guests are already turned away client-side and an anonymous upload can never become a product), L1 (stage throttle @ 5 rps), L2 (daily Gemini counter) and L4 (GCP billing detach), ~half a day. **L1 bounds the AWS bill; only L2 bounds the Google one** — Vertex quotas are per-minute, and a 1 rps upload route still permits 86,400 `gemini-3.5-flash` calls/day (~$311–1,089 at $1.50/$9.00 per M, with no `thinkingConfig` set anywhere in `server/src`). Geo-restriction is scoped as surface reduction, **not** a cost control: CloudFront bills a geo-blocked request at $1.20/M against API Gateway's $1.00/M, so it is cheaper above the free tier to let the flood hit the gateway; country-level `DE` over Baden-Württemberg (`DE-BW`), since German mobile carriers egress centrally and subdivision lookups would lock out the real team
