...
- `0003-always-on-production-cost-architecture.md` — *Proposed, gated.* Always-on ingress: drop the ALB for API Gateway HTTP API → VPC Link → Cloud Map (keeps Fargate warm; Lambda rejected on cold starts, App Runner on GHCR support). **Both stages run the same ingress** — `dev` is rebuilt onto it first as the rehearsal and stamped into `prod` from the same Terraform root via a second `.tfvars` (no conditional ingress resources); `dev` also becomes ephemeral rather than scheduled. **Blocked on step 0**: bound the synchronous Gemini plausibility/label-extraction calls with a nested timeout budget (20 s call / 25 s handler) — nothing bounds them today — then validate it with a synthetic batch, because HTTP APIs cap the integration timeout at a non-increasable 30 s (the ALB allows 60 s+). If the budget cannot hold, the async-upload rework is the answer and Cloudflare Tunnel the ingress fallback
