...
1. `requireOriginSecret` on `/api/*` (ADR 0005 Phase 2) — 403s any request lacking the `X-Origin-Verify` header the CloudFront distribution's WAF inserts on requests it allows through. A no-op when `ORIGIN_VERIFY_SECRET` is unset (local dev, tests, any stage with no CloudFront in front) — deliberately not fail-fast, unlike this file's other config vars.
2. `requestLogger` — emits one structured `request:start` (debug) and one `request:finish` (info/warn/error depending on status) line per request with `method`, `path`, `status`, `durationMs`, `userId`, `isAnonymous`, `ip`, and `x-request-id` (when supplied). Mounted before rate-limiting so even throttled requests are recorded.
3. Rate limiting: `apiLimiter` (100 req/15min) on `/api/*`, `authLimiter` (10 req/hr) on auth endpoints
4. `requireAuth` — verifies Supabase Bearer token, injects `user` into `req` (including `isAnonymous` flag derived from the JWT `is_anonymous` claim)
5. `requireRegistered` — composable second-layer guard for contribution routes; rejects anonymous sessions with `403 { error: 'Registration required' }`. Applied after `requireAuth` on `POST /api/products`, `POST /api/products/upload-image` (ADR 0005 L-1 — the plausibility gate runs a paid Gemini call before anything is persisted, so anonymous sessions never reach it), `POST /api/products/extract-label`, both `POST`/`DELETE /api/products/:barcode/verify`, and all P5-006 edit write paths (`PATCH /api/products/:barcode`, `POST /api/products/:barcode/edits`, `POST`/`DELETE /api/products/edits/:editId/votes`, `POST /api/products/edits/:editId/dismissals`).
6. `requestDeadline()` — 25 s handler deadline, mounted on the two synchronous Gemini routes (`POST /api/products/upload-image`, `POST /api/products/extract-label`). Answers `503 { code: 'request_timeout' }` if nothing has responded by then. It cannot *cancel* the handler — Express has no mechanism for that — so the handler may finish later and try to write to an already-sent response; `errorHandler` returns early on `res.headersSent` for exactly that reason. Paired with `withGeminiDeadline()` (`services/geminiDeadline.ts`), which bounds the model call itself at 20 s via `AbortSignal` and raises `503 { code: 'upstream_timeout' }`. Every *other* failure of that call is wrapped as `502 { code: 'upstream_error' }` — necessary because `@google/genai` throws an `ApiError` carrying Google's own HTTP status and `errorHandler` trusts `err.status`, so an unwrapped Vertex `404` ("model not published in this region") reached clients as our `404`, which in this API means "product not found". A dependency failing is never a 4xx. The budgets are nested — 20 s call < 25 s handler < the 30 s API Gateway integration timeout that ADR 0003's ingress would impose — so the innermost always fires first and a slow Gemini response is a clean, retryable 503 rather than an opaque 504 with the upload lost. Both 5xx bodies collapse to generic copy, so `code` is the only part a client can branch on.
7. Controllers handle request/response
8. `errorHandler` — centralized error middleware with a two-channel design. **Server side:** logs full detail (stack, Prisma `code`, `meta`, original message, path, method, userId) via winston. **Client side:** sanitized JSON body of shape `{ message, code? }`. 5xx errors and unknown Prisma errors collapse to a generic message (`"Something went wrong on our end. Please try again."`); the original `err.message` is only forwarded when the status is 4xx and the error does not set `expose: false`. Known Prisma codes are mapped to safe copy: `P2002` → 409 `unique_violation`, `P2003` → 409 `foreign_key_violation`, `P2025` → 404 `not_found`. The `AppError` interface exposes `status`, `code?`, and `expose?` for controllers that want stricter sanitisation.
